...
/*
Cysic Tetris App — now mobile-compatible with responsive design & touch controls
*/
...
export default function App() {
  ...
  function softDrop() {
    setCurrent(cur => {
      const nextPos = { x: cur.pos.x, y: cur.pos.y + 1 };
      if (!collides(board, cur.shape, nextPos)) return { ...cur, pos: nextPos };
      return cur;
    });
  }
  function hardDrop() {
    setCurrent(cur => {
      ...
      const { board: clearedBoard, cleared } = clearLines(
        merge(board, landed.shape, landed.pos, landed.color)
      );
      if (cleared > 0) {
        if (!mute) createBeep(880, 0.12, 0.08);
        setScore(s => s + [0, 40, 100, 300, 1200][cleared] * level);
  ...
  return (
    <div className="app-root">
      <header className="topbar">
        <div className="brand">
          <img src="/cysic-logo.png" alt="cysic logo" className="logo" />
          <h1>cysic Tetris</h1>
        </div>
      ...
      <main className="main-grid">
        ...
        <aside className="side-panel">
          <div className="stat"><div className="label">Score</div><div className="value">{score}</div></div>
          <div className="stat"><div className="label">Level</div><div className="value">{level}</div></div>
          <div className="stat"><div className="label">Lines</div><div className="value">{lines}</div></div>
      ...
      <footer className="footer">
        Built for the cysic community • v1.0 • By <a href="https://x.com/0xnald" target="_blank" rel="noopener noreferrer">@0xnald</a>
      </footer>
    </div>
  );
}
